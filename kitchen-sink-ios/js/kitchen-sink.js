...
// Removed Add another view, which is in right panel
...
myApp.onPageInit('modals', function (page) {
    $$('.demo-alert').on('click', function () {
        myApp.alert('<p class="text-red-error">Voith Red Alert</p>');
    });
...
pageContainer.find('.sign-in').on('click', function () {
	var username = $$('.login-screen-content').find('input[name="username"]').val();
	var password = $$('.login-screen-content').find('input[name="password"]').val();
